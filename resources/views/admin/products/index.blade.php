    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0"> / Products List </span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
@endsection
@section('content')
    <a class="btn d-block btn-outline-primary mb-3" href="{{ route('products.create') }}">Add Product</a>
                            ...
                            <tbody>
                                @foreach ($products as $index => $product)
                                    <tr>
                                        ...
                                        <td style="width: 400px">{{ $product->category?->title ?? 'Empty' }}</td>
                                        <td>
                                            <img src="{{ asset($product->image) }}" alt="course image"
                                                style="height: 70px;">
                                        </td>
                                        <td>
                                            <div class="row" style="gap: 10px; width: 170px">
                                                <a class="btn btn-primary" style="padding: 5px 10px" title="edit"
                                                    href="{{ route('products.edit', $product->id) }}">
                                                    <i class="fa fa-edit"></i>
                                                </a>
                                                <a class="btn btn-warning" style="padding: 5px 10px" title="images"
                                                    href="{{ route('product.images', $product->id) }}">
                                                    <i class="fa fa-image"></i>
                                                </a>
                                                {{-- <a class="btn btn-secondary" style="padding: 5px 10px" title="sizes"
                                                    href="{{ route('product.variants', $product->id) }}">
                                                    <i class="ti-ruler-alt"></i>
                                                </a> --}}
                                                <a class="modal-effect btn btn-danger employee-delete" title="delete"
                                                    style="padding: 5px 10px" data-effect="effect-slide-in-bottom"
                                                    data-toggle="modal" href="#delete" data-id="{{ $product->id }}">
