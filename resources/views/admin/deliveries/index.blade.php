...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">Deliveries </h4>
                <span class="text-muted mt-1 tx-13 mr-2 mb-0"> / Deliveries List </span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
@endsection
@section('content')
    <a class="btn d-block btn-outline-primary mb-3" href="{{ route('deliveries.create') }}">Add Delivery Area</a>
                        ...
                        <table id="example1" class="table key-buttons text-md-nowrap">
                            <thead>
                                <tr>
                                    ...
                                    <th class="wd-15p border-bottom-0">Delivery EN</th>
                                    <th class="wd-15p border-bottom-0">Delivery AR</th>
                            ...
                            <tbody>
                                @foreach ($deliveries as $index => $delivery)
                                    <tr>
                                        <td>{{ ++$index }}</td>
                                        <td style="width: 400px">{{ $delivery->getTranslation('name', 'en') }}</td>
                                        <td style="width: 400px">{{ $delivery->getTranslation('name', 'ar') }}</td>
                                        <td>LE {{ $delivery->cost }}</td>
                                        <td>
                                            <div class="row" style="gap: 10px; width: 120px">
                                                ...
                                                <a class="modal-effect btn btn-danger employee-delete"
                                                    ...
                                                    data-toggle="modal" href="#delete" data-id="{{ $delivery->id }}">
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content modal-content-demo">
                    <div class="modal-header">
                        <h6 class="modal-title">Delete Delivery area</h6><button aria-label="Close" class="close"
                            data-dismiss="modal" type="button"><span aria-hidden="true">×</span></button>
                    </div>
                    <form action="{{ route('deliveries.destroy', 'delete') }}" method="post">
