...
@section('page-header')
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">Cart Items</h4>
                <span class="text-muted mt-1 tx-13 mr-2 mb-0"> / Cart Items List </span>
            </div>
        </div>
    </div>
@endsection
                            ...
                            <tbody>
                                @foreach ($cartItems as $index => $item)
                                    <tr>
                                        <td>{{ ++$index }}</td>
                                        <td>
                                            @php($image = $item->preview_image_url ?: $item->image_url)
                                            @if ($image)
                                                <img src="{{ asset($image) }}" width="60" height="60" style="object-fit: contain">
                                            @else
                                                -
                                            @endif
                                        </td>
                                        <td>{{ $item->name }}</td>
                                        <td>{{ trim(($item->user->first_name ?? '') . ' ' . ($item->user->last_name ?? '')) ?: '-' }}</td>
                                        <td>{{ $item->quantity }}</td>
                                        <td>LE {{ $item->price }}</td>
                                        <td>
                                        ...
                                        <td style="text-wrap: nowrap;">{{ $item->created_at }}</td>
                                        <td>
                                            <div class="row" style="gap: 10px; width: 120px">
                                                <a class="btn btn-warning" style="padding: 5px 10px" href="{{ route('cart-items.show', $item->id) }}">
                                                    <i class="fa fa-eye"></i>
                                                </a>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content modal-content-demo">
                    <div class="modal-header">
                        <h6 class="modal-title">Delete Cart Item</h6><button aria-label="Close" class="close"
                            data-dismiss="modal" type="button"><span aria-hidden="true">x</span></button>
                    </div>
                    <form action="{{ route('cart-items.destroy', 'delete') }}" method="post">
