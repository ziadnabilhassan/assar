...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">Edit Review</h4>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
            ...
            <form action="{{ route('reviews.update', $review->id) }}" method="post" enctype="multipart/form-data">
                ...
                @foreach (LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                    <div class="form-group">
                        ...
                        <input value="{{ $review->getTranslation('name', $localeCode) }}" class="form-control"
                            placeholder="enter Title" name="name[{{ $localeCode }}]">
                    </div>
                    <div class="form-group">
                        <label for="overview">Review {{ $localeCode }} <span
                                class="font-weight-bold text-danger">*</span></label>
                        <textarea required class="form-control" placeholder="enter review" rows="5" name="text[{{ $localeCode }}]">{{ $review->getTranslation('text', $localeCode) }}</textarea>
                    </div>
                @endforeach
