    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0"> / Promo Codes List </span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
@endsection
@section('content')
    <a class="btn d-block btn-outline-primary mb-3" href="{{ route('promocodes.create') }}">Add Promo Code</a>
                        ...
                        <table id="example1" class="table key-buttons text-md-nowrap">
                            <thead>
                                <tr>
                                    ...
                                    <th class="wd-25p border-bottom-0">Max Use</th>
                                    <th class="wd-25p border-bottom-0">Uses count</th>
                            ...
                            <tbody>
                                @foreach ($promocodes as $index => $promocode)
                                    <tr>
                                        ...
                                        <td>{{ $promocode->discount_value }}
                                            {{ $promocode->discount_type == 'fixed' ? 'LE' : '%' }}</td>
                                        <td style="width: 50px">
                                        ...
                                        <td style="width: 100px">{{ $promocode->start_date->format('d-m-Y') }}</td>
                                        <td style="width: 100px">{{ $promocode->end_date->format('d-m-Y') }}</td>
                                        <td style="width: 100px">{{ $promocode->max_uses }}</td>
                                        <td style="width: 120px">{{ $promocode->uses_count }}</td>
                                        <td>
                                            <div class="row" style="gap: 10px; width: 120px">
                                                <a class="btn btn-primary employee-edit" style="padding: 5px 10px"
                                                    href="{{ route('promocodes.edit', $promocode->id) }}">
                                                    <i class="fa fa-edit"></i>
                                                </a>
                                                <a class="modal-effect btn btn-danger employee-delete"
                                                    style="padding: 5px 10px" data-effect="effect-slide-in-bottom"
                                                    data-toggle="modal" href="#delete" data-id="{{ $promocode->id }}">
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content modal-content-demo">
                    <div class="modal-header">
                        <h6 class="modal-title">Delete Promo code</h6><button aria-label="Close" class="close"
                            data-dismiss="modal" type="button"><span aria-hidden="true">×</span></button>
                    </div>
                    <form action="{{ route('promocodes.destroy', 'delete') }}" method="post">
